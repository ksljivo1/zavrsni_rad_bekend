...
export default function CFG(nonTerminals, alphabet, productions, startSymbol) {
  function removeEpsilonFromProduction(production, epsProductions) {
    ...
    if (countOfEpsProductions > 0) {
      const binStrings = new combinatorics.BaseN(
        [true, false],
        countOfEpsProductions
      );
  ...
  function getListOfGenerativeSymbols() {
    ...
    let newListOfGenerativeSymbols = nonTerminals.filter((symbol) => {
      for (const production of productions[symbol]) {
        if (
          production.every((symbol1) =>
            [...alphabet, EPSILON].includes(symbol1)
          )
        ) {
          return true;
        }
    ...
    while (
      newListOfGenerativeSymbols.length !== oldListOfGenerativeSymbols.length
    ) {
      ...
      newListOfGenerativeSymbols = nonTerminals.filter((symbol) => {
        for (const production of productions[symbol]) {
          if (
            production.every((symbol1) =>
              [...alphabet, ...oldListOfGenerativeSymbols, EPSILON].includes(
                symbol1
              )
            )
          ) {
  ...
  function getListOfReachableSymbols() {
    ...
    while (
      newListOfReachableSymbols.length !== oldListOfReachableSymbols.length
    ) {
      oldListOfReachableSymbols = [...newListOfReachableSymbols];
      oldListOfReachableSymbols
        .filter((symbol) => nonTerminals.includes(symbol))
        .forEach((symbol) => {
          ...
          newListOfReachableSymbols = [
            ...new Set([
              ...oldListOfReachableSymbols,
              ...newListOfReachableSymbols,
              ...setOfReachableSymbols,
            ]),
          ];
        });
    }
  ...
  function getListOfEmptySymbols() {
    ...
    let newListOfEmptySymbols = nonTerminals.filter((symbol) =>
      productions[symbol].some(
        (production) => production.length === 1 && production[0] === EPSILON
      )
    );

    while (newListOfEmptySymbols.length !== oldListOfEmptySymbols.length) {
      oldListOfEmptySymbols = newListOfEmptySymbols;
      newListOfEmptySymbols = [
        ...new Set([
          ...oldListOfEmptySymbols,
          ...nonTerminals.filter((symbol) => {
            return productions[symbol].some((production) =>
              production.every((symbol) =>
                oldListOfEmptySymbols.includes(symbol)
              )
            );
          }),
        ]),
  ...
  function removeNonGenerativeSymbols(
    nonTerminals,
    alphabet,
    productions,
    startSymbol
    ...
    const listOfNongenerativeSymbols = nonTerminals.filter(
      (symbol) => !newNonTerminals.includes(symbol)
    );
    ...
    for (const symbol of newNonTerminals) {
      ...
      for (const production of productions[symbol]) {
        const acceptRule = !production.some((symbol1) =>
          listOfNongenerativeSymbols.includes(symbol1)
        );
        if (acceptRule) {
          newProductions[symbol].push(production);
          production
            .filter((symbol1) => alphabet.includes(symbol1))
            .forEach((symbol1) => newAlphabet.add(symbol1));
        }
      }
  ...
  function removeNonReachableSymbols(
    nonTerminals,
    alphabet,
    productions,
    startSymbol
    ...
    return CFG(
      nonTerminals.filter((symbol) => listOfReachableSymbols.includes(symbol)),
  ...
  function getListOfDirectEpsilonProductions(
    listOfEmptySymbols,
    listOfProductions,
    nonTerminals
  ) {
    let listOfDirectEpsilonProductions = listOfEmptySymbols.filter(
      (production) =>
        listOfProductions[production].length === 1 &&
        listOfProductions[production][0].length === 1 &&
        listOfProductions[production][0][0] === EPSILON
    );

    let oldListOfDirectEpsilonProductions = [];
    while (
      listOfDirectEpsilonProductions.length !==
      oldListOfDirectEpsilonProductions.length
    ) {
      oldListOfDirectEpsilonProductions = [...listOfDirectEpsilonProductions];
      listOfDirectEpsilonProductions = [
        ...new Set([
          ...listOfDirectEpsilonProductions,
          ...nonTerminals.filter((symbol) =>
            productions[symbol].every((production) =>
              production.every((symbol) =>
                oldListOfDirectEpsilonProductions.includes(symbol)
              )
            )
          ),
        ]),
  ...
  function removeEpsilonProductions(grammar) {
    ...
    for (const symbol of newestNonTerminals) {
      ...
      newestListOfProductions[symbol].forEach((production) => {
        if (
          production.length === 1 &&
          [...newestAlphabet, EPSILON].includes(production[0])
        )
          newProductions.push(production);
        else
          newProductions.push(
            ...removeEpsilonFromProduction(production, listOfEmptySymbols)
          );
      });
    ...
    let listOfDirectEpsilonProductions = getListOfDirectEpsilonProductions(
      listOfEmptySymbols,
      newestListOfProductions,
      newestNonTerminals
    );
    ...
    for (const symbol of newestNonTerminals) {
      ...
      newestListOfProductions[symbol].forEach((production) => {
        newProductions.push(
          production.filter(
            (symbol1) =>
              ![...listOfDirectEpsilonProductions, EPSILON].includes(symbol1)
          )
        );
      });

      newestListOfProductions[symbol] =
        removeDuplicateProductions(newProductions);
    }

    let newNonTerminals = [];
    for (const symbol of newestNonTerminals) {
      if (newestListOfProductions[symbol].length === 0)
        delete newestListOfProductions[symbol];
      else newNonTerminals.push(symbol);
    }

    if (listOfEmptySymbols.includes(newestStartSymbol))
      newestListOfProductions[newestStartSymbol].push([EPSILON]);
  ...
  function unit(symbol, productions, nonTerminals) {
    ...
    while (
      newListOfUnitProductions.length !== oldListOfUnitProductions.length
    ) {
      oldListOfUnitProductions = [...newListOfUnitProductions];
      oldListOfUnitProductions.forEach((symbol) => {
        const newSymbol = productions[symbol].filter(
          (production) =>
            production.length === 1 && nonTerminals.includes(production[0])
        );
        if (newSymbol.length !== 0)
          newListOfUnitProductions.push(...newSymbol.flat());
        newListOfUnitProductions = removeDuplicateProductions(
          newListOfUnitProductions
        );
      });
    }
  ...
  function removeUnitProductions(grammar) {
      ...
      if (unitProductions.length > 1) {
        ...
        unitProductions.forEach((unitProduction) => {
          if (unitProduction !== symbol) {
            const newThing = productions[unitProduction].filter(
              (production) =>
                production.length !== 1 ||
                (production.length === 1 &&
                  newestAlphabet.includes(...production.flat()))
            );
            if (newThing.length !== 0) newProductions.push(...newThing);
          }
        });
        additionalProductions[symbol] =
          removeDuplicateProductions(newProductions);
      }
    });
    ...
    Object.keys(additionalProductions).forEach((symbol) => {
      ...
      newestListOfProductions[symbol] = removeDuplicateProductions(
        newestListOfProductions[symbol]
      );

      newestListOfProductions[symbol] = newestListOfProductions[symbol].filter(
        (production) => {
          return !(
            production.length === 1 &&
            unit(symbol, newestListOfProductions, newestNonTerminals).includes(
              ...production
            )
          );
        }
  ...
  function reduceToChomskyNormalForm() {
    ...
    const grammarWithoutNonGenerativeSymbols = removeNonGenerativeSymbols(
      nonTerminals,
      alphabet,
      productions,
      startSymbol
    ...
    const grammarWithoutNonReachableSymbols = removeNonReachableSymbols(
      grammarWithoutNonGenerativeSymbols.getNonTerminals(),
      grammarWithoutNonGenerativeSymbols.getAlphabet(),
      grammarWithoutNonGenerativeSymbols.getProductions(),
      grammarWithoutNonGenerativeSymbols.getStartSymbol()
    );

    // removing epsilon productions
    const { newestListOfProductions, newNonTerminals } =
      removeEpsilonProductions(grammarWithoutNonReachableSymbols);

    const grammarWithoutEpsilonProductions = CFG(
      newNonTerminals,
      [...grammarWithoutNonGenerativeSymbols.getAlphabet()],
      newestListOfProductions,
      grammarWithoutNonGenerativeSymbols.getStartSymbol()
    ...
    const grammarWithoutUnitProductions = removeUnitProductions(
      grammarWithoutEpsilonProductions
    );

    // removal of unit productions might result with useless productions
    const grammarWithoutUnitAndNonGenerativeProductions =
      grammarWithoutUnitProductions.removeNonGenerativeSymbols(
        grammarWithoutUnitProductions.getNonTerminals(),
        grammarWithoutUnitProductions.getAlphabet(),
        grammarWithoutUnitProductions.getProductions(),
        grammarWithoutUnitProductions.getStartSymbol()
      );

    const newestOfNewestGrammars =
      grammarWithoutUnitAndNonGenerativeProductions.removeNonReachableSymbols(
        grammarWithoutUnitAndNonGenerativeProductions.getNonTerminals(),
        grammarWithoutUnitAndNonGenerativeProductions.getAlphabet(),
        grammarWithoutUnitAndNonGenerativeProductions.getProductions(),
        grammarWithoutUnitAndNonGenerativeProductions.getStartSymbol()
      );
    ...
    newestOfNewestGrammars.getNonTerminals().forEach((symbol) => {
      const thiss = newestOfNewestGrammars
        .getProductions()
        [
          symbol
        ].filter((production) => (production.length === 1 && newestOfNewestGrammars.getAlphabet().includes(production[0])) || (production.length === 2 && production.every((symbol1) => newestOfNewestGrammars.getNonTerminals().includes(symbol1))));
      if (thiss.length !== 0) productionsPrime[symbol] = thiss;
    });

    const constructNewProductions = {};

    const listOfProductionsPrime = Object.values(productionsPrime).reduce(
    ...
    newestOfNewestGrammars.getNonTerminals().forEach((symbol) => {
      ...
      newestOfNewestGrammars.getProductions()[symbol].forEach((production) => {
        if (
          !listOfProductionsPrime.some((production1) =>
            production.every((symbol) => production1.includes(symbol))
          )
        ) {
          newProductions.push(
            production.map((symbol) => {
              if (newestOfNewestGrammars.getAlphabet().includes(symbol)) {
                constructNewProductions['REPLACEMENT_FOR_' + symbol] = [
                  [symbol],
                ];
    ...
    const newestOfNewestOfNonTerminals = [
      ...newestOfNewestGrammars.getNonTerminals(),
      ...Object.keys(constructNewProductions),
    ];
    newestOfNewestOfProductions = {
      ...newestOfNewestOfProductions,
      ...constructNewProductions,
    };
      ...
      newestOfNewestOfProductions[symbol].forEach((production, j) => {
        if (production.length >= 3) {
          ...
          for (k = 0; k < production.length - 2; k++) {
            ...
            k === 0
              ? replacementProductions[symbol].push([
                  production[k],
                  'D_' + i + '_' + j + '_' + k,
                ])
              : replacementProductions['D_' + i + '_' + j + '_' + (k - 1)].push(
                  [production[k], 'D_' + i + '_' + j + '_' + k]
                );
          }
          replacementProductions['D_' + i + '_' + j + '_' + (k - 1)] = [];
          replacementProductions['D_' + i + '_' + j + '_' + (k - 1)].push([
    ...
    return CFG(
      [...Object.keys(replacementProductions)],
      newestOfNewestGrammars.getAlphabet(),
      replacementProductions,
      newestOfNewestGrammars.getStartSymbol()
    );
  }
