export default function Parse(word, cfg1) {
    ...
    function getGenerativeTree() {
        ...
        if (!wordIsGeneratedByGrammar()) throw "Word is not generated by grammar!"

        const baseProductions = {}
        for (const production in cfg.getProductions()) {
            baseProductions[production] = cfg.getProductions()[production].filter(production1 => production1.length === 1).flat()
            if (baseProductions[production].length === 0) delete baseProductions[production]
        }
                        ...
                        if (production.length === 2) {
                            levels[levels.length - i + t].forEach(production1 => {
                                levels[levels.length - j - 1].forEach(production2 => {
                                    if (Object.keys(production1)[0] === production[0] && Object.keys(production2)[0] === production[1]) {
                                        levels[levels.length - i - 1].push({
                                            [nonTerminal]: [
                                                { [Object.keys(production1)[0]]: Object.values(production1)[0] },
                                                { [Object.keys(production2)[0]]: Object.values(production2)[0] }
                                            ]
                                        })
                                    }
                                })
